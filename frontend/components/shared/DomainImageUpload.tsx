...
export function DomainImageUpload (props : DomainImageUploadProps) {
  ...
  const [croppedImgUrl, setCroppedImgUrl] = React.useState<string | null>(() => {
    if(props.startingSrc) return props.startingSrc;
    return null;
  });
  const [cropping, setCropping] = React.useState<boolean>(false);

  React.useEffect(() => {
    if (props.startingSrc) setCroppedImgUrl(props.startingSrc);
  }, [props.startingSrc])
